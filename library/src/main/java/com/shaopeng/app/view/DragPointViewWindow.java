...
import com.shaopeng.app.tools.MathTool;
...
class DragPointViewWindow extends AbsDragPointView implements ValueAnimator.AnimatorUpdateListener, Animator.AnimatorListener {
    ...
    private void drawBezierLine(Canvas canvas) {
        ...
        if (dx != 0) {
            float k1 = dy / dx;
            float k2 = -1 / k1;
            mDragTangentPoint = MathTool.getIntersectionPoints(
                    mDragCircle.x, mDragCircle.y, mDragRadius, (double) k2);
            mCenterTangentPoint = MathTool.getIntersectionPoints(
        ...
        if (mRecoveryAnim == null || !mRecoveryAnim.isRunning()) {
            switch (event.getAction()) {
                ...
                case MotionEvent.ACTION_MOVE:
                    ...
                    mDistanceCircles = MathTool.getDistance(mCenterCircle, mDragCircle);
    ...
    private void upAndCancelEvent() {
        if (isInCircle && mDistanceCircles == 0) {
            ...
            if (mOnPointDragListener != null) {
                mOnPointDragListener.onPointRecovery(this);
            }
        } else if (!mIsDragOut) {
    ...
    public void startRemove() {
        if (mRemoveAnim == null) {
            ...
            if (mOnPointDragListener != null) {
                mOnPointDragListener.onPointRemoveStart(this);
                mOnPointDragListener.onPointRemoveEnd(this);
            }
        } else {
